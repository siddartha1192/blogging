...
# Add this global variable for stop signal
STOP_TRADING = False
STOP_FILE_PATH = 'trading_stop_signal.txt'

def signal_handler(signum, frame):
    """Handle termination signals"""
    global STOP_TRADING
    print("Received termination signal. Stopping trading...")
    logging.info("Received termination signal. Stopping trading...")
    STOP_TRADING = True
...
def cleanup_and_exit():
    ...
    logging.info("Trading script stopped cleanly")
        ...
        if start_time_ist <= current_time_ist <= end_time_ist:
            try:
                ...
                if not hist_data.empty:
                    ...
                    if STOP_TRADING or check_stop_signal():
                        break
                ...
                print(f"Error: {e}")
                # Continue loop even on error, but check stop signal
                if STOP_TRADING or check_stop_signal():
                    break
